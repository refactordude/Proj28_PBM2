...
hypothesis: RESOLVED — both root causes confirmed and fixes verified by user in browser.
test: user confirmed both bugs eliminated after commit a1d2c27.
expecting: n/a
...
verification: |
  User confirmed "confirm fixed" after commit a1d2c27 (2026-04-29).
  Both bugs eliminated in browser:
    - Chip clicks now populate and submit the Ask form correctly.
    - Confirm parameters dropdown renders without clipping.

  Lesson learned: the prior session diagnosed Bug #1 as a missing requestSubmit() call
  by reading only the template source. The real cause (tojson double-quote inside a
  double-quoted HTML attribute) was invisible from source inspection alone because the
  template itself looks plausible. Only examining the *rendered* HTML via curl revealed
  that the onclick attribute was terminated mid-expression by the embedded double-quote.
  Going forward: when an inline event handler involves dynamic string interpolation,
  always verify the rendered output — never trust the template appearance alone.
files_changed:
