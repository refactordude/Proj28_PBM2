...
### Phase 5: Stacked-Panel Filter/Grid Separation on Joint Validation + Browse — Split the existing single-panel layout on /overview (Joint Validation) and /browse into two visually-separate stacked panels: a slim filter panel (.panel containing the existing _filter_bar.html) above, and a grid panel (.panel containing .ph header + .panel-body grid + .panel-footer) below, separated by a ~16px vertical gap. Filter-bar wraps in its own .panel rather than sitting inside the grid panel's .panel-body. Preserve all HTMX OOB contracts: #overview-grid, #grid-count, #overview-count, #overview-pagination, filter_badges_oob, picker_badges_oob, params_picker_oob receivers all keep their ids and stay merge-by-id stable. Visual tokens come from existing tokens.css; only app.css + the two index.html templates change. AI Summary modal, presets chip strip, and active-filter chip rows stay in their current relative position (between the two panels or attached to the filter panel — TBD in discuss).
