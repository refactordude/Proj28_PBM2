...
- [ ] **INFRA-06**: **Pre-work gate — ufs_service refactor.** `app/services/ufs_service.py` is refactored so `list_platforms`, `list_parameters`, `fetch_cells`, and `pivot_to_wide` each have a pure `_core()` variant callable WITHOUT Streamlit context. The existing `@st.cache_data` wrappers delegate to the `_core()` functions so v1.0 API stays unchanged. All 171 v1.0 tests must still pass after refactor.
...
- [ ] **INFRA-09**: Shared `requirements.txt` is extended with v2.0 deps (`fastapi>=0.136,<0.137`, `uvicorn[standard]>=0.32`, `jinja2>=3.1`, `jinja2-fragments>=1.3`, `python-multipart`, `markdown-it-py[plugins]>=3.0`, `cachetools>=7.0,<8.0`, `pydantic-settings>=2.14`). Existing Streamlit deps are retained.
...
- [ ] **SUMMARY-05**: Summary session cache: `cachetools.TTLCache(maxsize=128, ttl=3600)` keyed by `(platform_id, content_mtime, llm_name, llm_model)`. Subsequent clicks on the same entity return cached result instantly unless the content file's `mtime` changed.
- [ ] **SUMMARY-06**: Regenerate button appears next to the rendered summary. Clicking bypasses the cache (via `hx-headers={"X-Regenerate": "true"}`) and forces a fresh LLM call.
- [ ] **SUMMARY-07**: Error state: if the LLM call fails (network, auth, timeout), the summary div shows a Bootstrap alert ("Summary unavailable: {reason}. Try again or switch LLM backend in Settings.") with a Retry button. No silent failure.
