...
# (260507-rmj reduced from 6 by dropping Status; 260507-s5c restored to 6
# by adding AP Model between AP Company and Device — net facet set differs
# from pre-260507-rmj)
# ---------------------------------------------------------------------------


def test_jv_filter_bar_uses_picker_popover_macro_unmodified() -> None:
    """D-JV-11 (post-260507-s5c): filter bar imports picker_popover from browse/_picker_popover.html."""
    ...
    assert fb.count("picker_popover(") >= 6, (
        "Filter bar must invoke picker_popover 6 times "
        "(D-JV-11 post-260507-s5c — AP Model added; post-260507-rmj — Status dropped)"
    )
